...
function install_video_drivers() {
    ...
    user_gpu=$(echo "$user_gpu" | tr '[:lower:]' '[:upper:]')  # Convertit la saisie en majuscules

    while [[ -z "$user_gpu" || ! " INTEL AMD NVIDIA VM " =~ " $user_gpu " ]]; do
        read -p "Choix de carte graphique invalide. Veuillez entrer INTEL, AMD, NVIDIA ou VM:" user_gpu
        user_gpu=$(echo "$user_gpu" | tr '[:lower:]' '[:upper:]')  # Convertit la saisie en majuscules
    done
