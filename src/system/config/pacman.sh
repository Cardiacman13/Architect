...
function mirrorlist() {
    ...
    exec_log "sudo reflector --verbose --score 100 --latest 20 --fastest 5 --sort rate --save /etc/pacman.d/mirrorlist" "Updating mirrorlist"
    exec_log "sudo pacman -Syy" "Updating pacman database ${RED}(might be long)${RESET}"
}
